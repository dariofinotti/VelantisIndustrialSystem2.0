...
def _read_sfdx_access_token(username: str) -> str:
    """Read a fresh access token from the SF CLI .sfdx auth file for the given username.

    CCI's org_config.access_token is unavailable (returns a redacted placeholder) for
    connected orgs whose CCI keychain session has expired. SFDMU v5 requires a valid token
    in the export.json orgs block, so we fall back to the SF CLI's own auth store.
    """
    sfdx_file = os.path.expanduser(f"~/.sfdx/{username}.json")
    if os.path.isfile(sfdx_file):
        try:
...
class LoadSFDMUData(SFDXBaseTask):
    ...
    def _prepare_export_json_file(self) -> None:
        ...
        try:
            ...
            if token and "REDACTED" not in str(token):
                org_data = {
                ...
                # No valid access token available — leave orgs empty so SFDMU v5
                # authenticates via the --targetusername CLI flag using SF CLI's own
                # credential store (the org must already be authenticated in SF CLI).
                export_json["orgs"] = []
                self.logger.info("No valid access token; SFDMU will authenticate via SF CLI credential store")
    ...
    def _prep_runtime(self) -> None:
        ...
        self.targetusername = self.options.get("targetusername") or self.org_config.username
